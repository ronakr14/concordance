...
|---|---:|---|---|---|
| `exact_npi` | 800 | `MATCH` | The sanction record carries the provider's real, checksum-valid NPI. Other fields may still be corrupted. | The deterministic path. If this is not ~100% precise and recallable, nothing downstream is trustworthy. |
...
| `ambiguous` | 400 | `AMBIGUOUS` | Drawn from a planted twin or common-name cluster, then stripped of NPI, DOB and street address — several providers fit equally well. | Overconfidence. The correct behaviour is to *refuse* to auto-decide and route to review; `corruption_profile.plausible_provider_ids` lists every defensible answer. |
| `false_positive_bait` | 400 | `NO_MATCH` | Same first and last name, same city and state as a real provider — but a different person, with a different DOB (9–25 years apart) and a different valid NPI. The person is **not** in the master. | Recall-chasing. Every one of these that is matched is a provider wrongly flagged as excluded, which is the expensive error in this domain. |
| `unmatched` | 400 | `NO_MATCH` | A complete, plausible, checksum-valid record for somebody who simply is not in the provider file. | The threshold. A system with no `NO_MATCH` outcome scores beautifully and is useless. |
...
| `common_name` | Two or three people sharing a first and last name inside one state, different addresses and DOBs. | Blocking on name and state returns all of them; the posterior has to come from the remaining fields. |
...
| `address` | USPS abbreviation, unit/suite dropped, ZIP+4 instead of ZIP5, wrong ZIP, PO box substituted, whole address missing | Practice addresses that change, mailing vs service addresses, and normalization applied by one system and not the other. |
