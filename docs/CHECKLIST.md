...
### GATE 6
- [x] 5,000 sanction records reconciled against 50,000 providers in an acceptable wall time — **record the number** ⭐ — **36 min 49 s** end to end (2026-09-21, `concordance_demo`, run `d7a70e52`), repeated at a similar time on the rebuild, across the WAN link to Neon. That is the link, not the engine: the same 50k × 5k work in process takes **124 s** (`make perf`). Both runs' counts — 3,412 / 967 / 621 — are exactly the column totals of the in-process evaluation's confusion matrix, so the Postgres path decides as the Parquet path does. Getting there found two faults: a connection Neon drops mid-insert failed the whole run, and the reconnect could then fail on a DNS lookup that this machine's network drops intermittently. A chunk is now rewritten after a reconnect from the writer state it started with, DNS failures count as disconnects, and a failed run still records itself as FAILED
...
- [x] Full suite green on a second machine — CI, since there is no container to prove host-independence ⭐ — green on GitHub's runner since 2026-09-22 — this is what "all tests run inside Docker" was for: catching a suite that only passes on the machine that wrote it
...
- [x] CI green on the default branch ⭐ — run 35697945496 on `main` (`c2c8717`), 2026-09-22: lint, credential scan, typecheck, migrate, preflight, unit, full suite with both coverage gates, the reset test, the web build and the dependency audit. Four runs to get there; each failure was CI setup, not application code
